...
async def common_tts_logic(texts, prompt: str, wav_path: str, speed: float = 1.0, stream: bool = False):
    """
    共通のTTS処理ロジック
        ...
        if stream:
            ...
            try:
                ...
                if not audio_chunks:
                    ...
                    raise HTTPException(status_code=500, detail=f"Audio generation failed: {str(gen_error)}")
                # 部分的に生成された場合は警告ログで継続
